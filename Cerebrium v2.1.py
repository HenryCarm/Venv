...
class CerebriumApp(ctk.CTk):
    ...
    def get_perfect_bubble_size(self, text):
        ...
        for line in lines:
            ...
            else: display_lines += math.ceil((len(line) * 8.5) / (box_width - 25))
            
        box_height = max(35, display_lines * 22 + 25)
    ...
    def add_bubble(self, sender, text="", idx=None, model_info=None, is_typing=False):
        ...
        if not is_typing:
            btn_f.pack(anchor="w" if btn_f._is_cel else "e", padx=15, pady=(0, 6))
    ...
    def ask_ai(self, msg):
        try:
            ...
            if model == "offline-model":
                ...
                self.dev_log(f"Sending formatted payload to offline brain... 🧠")
                comp = self.local_llm(raw_prompt, max_tokens=1500, stop=stop_tokens, echo=False)
    ...
    def type_text(self, box_widget, btn_f, full_text, current_text="", index=0):
        ...
        if index < len(full_text):
            ...
            if btn_f: btn_f.pack(anchor="w" if getattr(btn_f, '_is_cel', False) else "e", padx=15, pady=(0, 6))
